document.addEventListener('DOMContentLoaded', () => {
    ...
    for (const key in keyCode[`${lang}`]) {
      if (elem.dataset.code === key) {
        if (typeof keyCode[`${lang}`][key] !== 'object') {
          ...
          const shiftDown = document.createElement('span');
  ...
  class CreateKeyWrapper {
    ...
    createKeyWrap() {
      ...
      const keyLinesBlock = new CreateKeyLines();
      let count = 0;
      while (count < 6) {
      ...
      const numberArray = [8, 13, 13, 14, 14, 14];
      const keyItemBlock = new CreateKeyItem();
      let valueCounter = 0;
      for (let i = 0; i < keyWrap.children.length; i++) {
        let n = numberArray[numberArray.length - 1];
        while (n > 0) {
          const keyboardItem = keyItemBlock.createItem();
